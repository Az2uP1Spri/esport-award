// Vérifier que ESPORT_DATA est disponible
if (typeof ESPORT_DATA === 'undefined') {
  console.error('ESPORT_DATA n\'est pas défini. Assurez-vous que data/esportData.js est chargé avant vote.js');
}

document.addEventListener("DOMContentLoaded", () => {
  // Vérifier que les données sont disponibles
  if (!ESPORT_DATA || !ESPORT_DATA.games) {
    console.error('Les données esport ne sont pas disponibles');
    return;
  }

  const games = ESPORT_DATA.games;
  ...
  const unrankedGamesContainer = document.getElementById("unranked-games");
  ...
  // Générer les filtres de jeux
  function generateGameFilters() {
    let html = '<button class="filter-btn active" data-game="all">Tous les jeux</button>';
    games.forEach(game => {
      html += `<button class="filter-btn" data-game="${game.id}">${game.name}</button>`;
    });
    gameFiltersContainer.innerHTML = html;
  }

  // Générer les sections de jeux avec catégories et nominés
  function generateGameSections() {
    ...
    games.forEach(game => {
      ...
      game.categories.forEach(category => {
        ...
        category.nominees.forEach(nominee => {
          ...
          if (nominee.team) {
            html += `<p class="nominee-team">${nominee.team}</p>`;
          }
          
          html += `
                <p class="nominee-role">${nominee.roleDescription}</p>
                <button class="vote-btn">Sélectionner ce nominé</button>
              </div>
            </div>
          `;
        });
    ...
    gameSectionsContainer.innerHTML = html;
  }

  // Générer la tier list avec les jeux non classés
  function generateTierList() {
    ...
    unrankedGamesContainer.innerHTML = html;
  }
